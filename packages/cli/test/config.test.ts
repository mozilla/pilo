...
import { type SparkConfigResolved, getConfigDefaults } from "spark-core/config/defaults.js";

// Get defaults for creating complete mock configs
const defaults = getConfigDefaults();

// Mock the entire config module to test the interface
vi.mock("spark-core/config/configManager.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("spark-core/config/configManager.js")>();
  const mockConfig = {
    getConfig: vi.fn(),
