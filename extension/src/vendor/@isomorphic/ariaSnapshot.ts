...
export function parseAriaSnapshot(
  yaml: YamlLibrary,
  text: string,
  options: yamlTypes.ParseOptions = {},
  ...
  const convertMap = (container: AriaTemplateRoleNode, map: yamlTypes.YAMLMap) => {
    for (const entry of map.items) {
      ...
      if (key.value === "/children") {
        ...
        if (
          !valueIsString ||
          (value.value !== "contain" && value.value !== "equal" && value.value !== "deep-equal")
        ) {
          errors.push({
  ...
  if (fragment.children?.length === 1) return { fragment: fragment.children[0], errors };
