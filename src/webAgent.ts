...
export class WebAgent {
  ...
  private async handleBrowserDisconnect(
    ...
    // Reset message history so the agent re-executes the plan on the new browser.
    ...
    const data: Omit<BrowserReconnectedEventData, "timestamp" | "iterationId"> = {
      ...
      // nextStartIndex is i+1 after a successful connectOverCDP on endpoint i,
      // so it equals the 1-based index of the endpoint just used
      endpointIndex: browserAny.nextStartIndex ?? 0,
      total: browserAny.pwCdpEndpoints?.length ?? 0,
    };
